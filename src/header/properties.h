...
struct TimeProps {
	...
	//! the current number of iterations in dual solution
	int adjiter;
	...
	void incrtime(double *indt) {
		...
		if (time + *indt > ndnextoutput)
			*indt = ndnextoutput - time;
		if (time + *indt > ndnextsave)
			*indt = ndnextsave - time;
		if (time + *indt > ndmaxtime)
			*indt = ndmaxtime - time;
		dtime = *indt;
		// then increment time
		time += *indt;
	...
	int ifadjoint_out() {
		int output = fabs(remainder(time, timeoutput / TIME_SCALE)) < 1.0E-06 ? 1 : 0;
		if (output)
			time = ((double) rint(time * TIME_SCALE / timeoutput)) * timeoutput / TIME_SCALE;
		return output;
	}
...
struct ResFlag {
	...
	//this flag shows from which part of the code calc_edge_state is being called
	int callflag;

	//this flag shows prev_state_vars[0] before perturbation is less than GEOFLOW_TINY or not
	int lgft;
};
...
struct PertElemInfo {
	...
	double elem_prev_state[6];

	int iter;

	double forw_functional;

	double dual_functional;

	double perturb_functional;

	double dual_sens;

	vector<double> forw_func;
	vector<double> dual_func;
	vector<double> pert_func;
	...
	PertElemInfo(double* pos, double hmax, int iter_i) {
		...
		for (int i = 0; i < 6; i++)
			elem_state[i] = elem_prev_state[i] = 0.;
		...
		for (int i = 0; i < 5; i++)
			for (int j = 0; j < 3; j++) {
				...
				neigh_jac[i].curr_adj[j] = 0.;
				neigh_jac[i].prev_adj[j] = 0.;

				for (int k = 0; k < 3; k++)
					neigh_jac[i].jacobianMat[j][k] = 0.;
			}

		forw_functional = 0.;
		dual_functional = 0.;
		perturb_functional = 0.;
		h = hmax;
		func_sens = 0.;
		dual_sens = 0.;
	...
	void dumpinfo() {
		...
		char filename[50];
		sprintf(filename, "adjoint.test.%04d", iter);
		FILE *fp = fopen(filename, "w");

		fprintf(fp, "functionals: forward_run=%1.8e , dual_run=%1.8e, perturb_run=%1.8e \n",
		    forw_functional, dual_functional, perturb_functional);

		fprintf(fp, "forward sensitivity=%16e \n", (perturb_functional - forw_functional) / INCREMENT);

		fprintf(fp,
		    "\n ==============================This is the part that we have to care about========= \n");

		fprintf(fp, "functional sensitivity= %1.8e \n", func_sens);

		fprintf(fp, "dual       sensitivity= %1.8e \n", dual_sens);

		fprintf(fp, "Element position x= %1.8e y=%1.8e \n", elempos[0], elempos[1]);

		fprintf(fp, "Element size dx= %1.8e dy=%1.8e \n", elem_size[0], elem_size[1]);

		for (int effelement = 0; effelement < 5; effelement++) {

			fprintf(fp, "\nAdjoint and  Jacobian information for neighbor # %d:\n \n", effelement);

			for (int i = 0; i < 3; i++)
				fprintf(fp, "curr_adjoint[%d]= %1.8e  ", i, neigh_jac[effelement].curr_adj[i]);

			fprintf(fp, "\n");

			for (int i = 0; i < 3; i++)
				fprintf(fp, "prev_adjoint[%d]= %1.8e  ", i, neigh_jac[effelement].prev_adj[i]);
			...
			for (int i = 0; i < 3; i++) {
				fprintf(fp, "\n");
				for (int j = 0; j < 3; j++)
					fprintf(fp, "jacobian[%d][%d]= %1.8e  ", i, j, neigh_jac[effelement].jacobianMat[i][j]);
			}
	...
	void update_forw_func(double func) {

		forw_functional = func;
		forw_func.push_back(func);

	}

	void update_pert_func(double func) {

		perturb_functional = func;
		pert_func.push_back(func);

	}

	void update_dual_func(double func) {

		dual_functional = func;
		dual_func.push_back(func);

	}

	void dump_functional(TimeProps* timep) {

		char filename[50];
		sprintf(filename, "functional.test.%04d", iter);

		if (dual_func.size() == forw_func.size() && dual_func.size() == pert_func.size()) {

			FILE *fp = fopen(filename, "w");

			for (int i = 0; i < dual_func.size(); i++)
				if (i == 0)

					fprintf(fp, "functionals: forward_run=%16e , dual_run=%16e, perturb_run=%16e \n",
					    forw_func.at(i), dual_func.at(i), pert_func.at(i));
				else
					fprintf(fp, "functionals: forward_run=%16e , dual_run=%16e, perturb_run=%16e \n",
					    forw_func.at(i) - forw_func.at(i - 1), dual_func.at(i) - dual_func.at(i - 1),
					    pert_func.at(i) - pert_func.at(i - 1));
	...
	void compute_sensitivity() {
		...
		for (int neigh = 0; neigh < 5; ++neigh)
			for (int ind = 0; ind < 3; ++ind)
				contr += neigh_jac[neigh].jacobianMat[0][ind] * neigh_jac[neigh].prev_adj[ind];

		dual_sens = neigh_jac[0].curr_adj[0] + contr;
