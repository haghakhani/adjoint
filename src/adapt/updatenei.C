	...
	if (EmDebug) {
		if (EmDebugFather.get_num_elem() > 0) {
			fprintf(fp, "EmDebug={%10u,%10u} has %d Father(s)\n", EmDebug->key[0],
					EmDebug->key[1], EmDebugFather.get_num_elem());
			for (int iFather = 0; iFather < EmDebugFather.get_num_elem(); iFather++) {
				fprintf(fp,
						...
						iFather, *(EmDebugFather.get_key(iFather) + 0),
						*(EmDebugFather.get_key(iFather) + 1),
						...
						EmDebugFather.get(iFather)->Awet, EmDebugFather.get(iFather)->Swet,
		...
		fprintf(fp, "drypoint={%9.6f,%9.6f} ", EmDebug->drypoint[0],
				EmDebug->drypoint[1]);
		fprintf(fp, "has neighbors (%d are unique)\n", uniqueneigh);
		for (ineigh = 0; ineigh < 8; ineigh++)
			fprintf(fp, " %d:   {%10u,%10u}  proc=%d gen=%d\n", ineigh,
					EmDebug->neighbor[ineigh][0], EmDebug->neighbor[ineigh][1],
		...
		if (EmDebugNeigh.get_num_elem() > 0) {
			...
			for (ineigh = 0; ineigh < EmDebugNeigh.get_num_elem(); ineigh++)
				fprintf(fp,
						" %d:   {%10u,%10u}  proc=%d gen=%d adapted=%d which_son=%d iwetnode=%d Awet=%9.6f Swet=%9.6f drypoint={%9.6f,%9.6f}\n",
						ineigh, *(EmDebugNeigh.get_key(ineigh) + 0),
...
int ifCheckNode(HashTable* El_Table, HashTable* NodeTable, int NdDebugInfo,
		int checkstate, unsigned *elemdbkey, Element *EmFather, Element *EmSonA,
		Element *EmSonB, Element *EmNeighNew[4]) {
	int ifcheck = 0;

	if ((checkstate == 0) || (NdDebugInfo == checkstate)) {
...
void AssertMeshErrorFree(HashTable *El_Table, HashTable* NodeTable,
	...
	for (ielembucket = 0; ielembucket < El_Table_num_buck; ielembucket++) {
		...
		while (El_Table_entry_ptr) {
			...
			assert(
					!((EmTemp->get_refined_flag()>=1)^ ((EmTemp->get_adapted_flag()==TOBEDELETED)|| (EmTemp->get_adapted_flag()==OLDFATHER)|| (EmTemp->get_adapted_flag()==OLDSON) ) ));

			//^ is "exclusive or", not "exclusive or" is "if and only if"
			assert(
					!((EmTemp->get_refined_flag()==GHOST)^ ((EmTemp->get_adapted_flag()>=-BUFFER)&& (EmTemp->get_adapted_flag()<=-NOTRECADAPTED) ) ));

			//^ is "exclusive or", not "exclusive or" is "if and only if"
			assert(
					!((EmTemp->myprocess!=myid)^ ((EmTemp->get_adapted_flag()>=-BUFFER)&& (EmTemp->get_adapted_flag()<=-NOTRECADAPTED) ) ));
			...
			//^ is "exclusive or", not "exclusive or" is "if and only if"
			assert(
					(EmTemp->get_refined_flag()==0)^ !((EmTemp->get_adapted_flag()>=NOTRECADAPTED)&& (EmTemp->get_adapted_flag()<=BUFFER) ));
			...
			double xmax = -HUGE_VAL, xmin = HUGE_VAL, ymax = -HUGE_VAL, ymin =
					HUGE_VAL;
			double *coord;
			...
			if (EmTemp->get_adapted_flag() >= NOTRECADAPTED) {
				...
				coord = NdTemp->get_coord();
				assert(
						(coord[0] - tol <= xmin) && (xmin <= coord[0] + tol)
				...
				coord = NdTemp->get_coord();
				assert(
						(coord[0] - tol <= xmax) && (xmax <= coord[0] + tol)
				...
				coord = NdTemp->get_coord();
				assert(
						(coord[0] - tol <= xmean) && (xmean <= coord[0] + tol)
				...
				for (iside = 0; iside < 4; iside++) {
					...
					if (EmTemp->neigh_proc[ineigh] >= 0) {
						...
						EmNeigh[0] = (Element *) El_Table->lookup(EmTemp->neighbor[ineigh]);
						if (!EmNeigh[0]) {
							//if(myid==TARGETPROC) {
						...
						EmNeigh[1] = (Element *) El_Table->lookup(
						...
						for (i = 0; i < 2; i++)
							if (EmNeigh[i]->adapted >= NOTRECADAPTED) {
								...
								for (ineighme = 0; ineighme < 8; ineighme++)
									if (compare_key(EmNeigh[i]->neighbor[ineighme], EmTemp->key))
										break;
								...
								if (!(EmTemp->neigh_gen[ineigh + 4 * i]
									...
									fprintf(fp, "EmTemp={%10u,%10u}\n", *(EmTemp->pass_key() + 0),
											*(EmTemp->pass_key() + 1));
									ElemBackgroundCheck(El_Table, NodeTable, EmTemp->pass_key(),
											fp);
									fprintf(fp, "EmNeigh={%10u,%10u} ineigh=%d\n",
											*(EmNeigh[i]->pass_key() + 0),
											*(EmNeigh[i]->pass_key() + 1), ineigh + 4 * i);
									ElemBackgroundCheck(El_Table, NodeTable,
											EmNeigh[i]->pass_key(), fp);
									fclose(fp);

									assert(
											EmTemp->neigh_gen[ineigh + 4 * i]
													== EmNeigh[i]->generation);
								}
								...
								if (EmTemp->get_adapted_flag() >= NOTRECADAPTED) {
									...
									case -1:
										...
										case 0: //neighbor is ineigh=iside
											...
											assert(
													compare_key(EmTemp->node_key[ineighp4],
															EmNeigh[0]->node_key[ineighme]));
											NdTemp = (Node*) NodeTable->lookup(
													EmTemp->node_key[ineighp4]);
											assert(NdTemp);

											if (!(NdTemp->info == S_C_CON)) {
												//printf("myid=%d\n",myid);
												//if(myid==TARGETPROC)
												NodeBackgroundCheck(El_Table, NodeTable, NdTemp->key,
														fp);
												fclose(fp);
										...
										case 1: //neighbor is ineighp4=iside+4
											...
											assert(
													compare_key(EmTemp->node_key[(ineigh + 1) % 4],
									...
									case 0:
										...
										if (!compare_key(EmTemp->node_key[ineigh],
											...
											ElemBackgroundCheck(El_Table, NodeTable, EmTemp->key, fp);
											fprintf(fp, "[i=0]\n");
											ElemBackgroundCheck(El_Table, NodeTable, EmNeigh[0]->key,
													fp);
											fprintf(fp, "[i=1]\n");
											ElemBackgroundCheck(El_Table, NodeTable, EmNeigh[1]->key,
													fp);
											//fprintf(fp,"stop me\n");
											fclose(fp);
											assert(
									...
									case 1:
										...
										case 1:
											if (!compare_key(EmTemp->node_key[ineigh],
												...
												fprintf(fp, "iside=%d ineigh=%d ineighme=%d i=%d\n",
														iside, ineigh, ineighme, i);
												ElemBackgroundCheck(El_Table, NodeTable, EmTemp->key,
														fp);
												fprintf(fp, "[i=0]\n");
												ElemBackgroundCheck(El_Table, NodeTable,
														EmNeigh[0]->key, fp);
	...
	for (inodebucket = 0; inodebucket < NodeTable_num_buck; inodebucket++) {
		...
		while (NodeTable_entry_ptr) {
			...
			assert(
					(NdTemp->info == CORNER) || (NdTemp->info == SIDE)
							|| (NdTemp->info == S_S_CON) || (NdTemp->info == BUBBLE)
							|| (NdTemp->info == S_C_CON));
...
int IfNeighProcChange(HashTable* El_Table, HashTable* NodeTable, int myid,
		Element* EmDebug, Element* EmTemp) {
	if (myid == 2)
...
#else
void refine_neigh_update(HashTable* El_Table, HashTable* NodeTable, int nump,
	...
	Element* ElemDebugKidnapper = (Element*) El_Table->lookup(
			ElemDebugKidnapperKey);
	...
	if (0 && myid == 2) {
		...
		if (timeprops_ptr->iter == 19) {
			...
			if (ElemDebugFather) {
				...
				ineigh = -1;
				if (ElemDebug->which_son == 3)
					ineigh = 3;
				else if (ElemDebug->which_son == 0)
					ineigh = 7;
				if (ineigh != -1) {
					ElemDebugFatherNeighKey[0] = ElemDebug->neighbor[ineigh][0];
					ElemDebugFatherNeighKey[1] = ElemDebug->neighbor[ineigh][1];
					ElemDebugFatherNeigh = (Element*) El_Table->lookup(
							ElemDebugFatherNeighKey);
				}
			...
			if (ElemDebugFatherNeigh) {
				...
				ElemBackgroundCheck(El_Table, NodeTable, ElemDebugFatherNeighKey,
						fpdb2);
			}
			fprintf(fpdb2,
					"**********************************************************\n");
				...
				if ((neigh_proc >= 0) && (neigh_proc != myid)) {
					...
					case -1:
						...
						EmSon[isonA]->neigh_gen[ineighm4] =
								EmSon[isonA]->neigh_gen[ineighp4] = EmSon[isonA]->generation;

						EmSon[isonA]->neigh_proc[ineighm4] = neigh_proc;
						EmSon[isonA]->neigh_proc[ineighp4] = -2;

						NdTemp = (Node*) NodeTable->lookup(
								EmSon[isonA]->node_key[ineighp4]);
						assert(NdTemp);
						NdTemp->info = SIDE;
						...
						if (EmFather->neigh_proc[ineighother] == myid) {
							...
							EmNeighNew[0] = EmNeighOld[0] = (Element*) El_Table->lookup(
					...
					case 0: //old neighbor was same generation as OLDFATHER and is
						...
						if (ineigh < 4) {	  //only do once, this if should not be necessary
							...
							if (!EmSon[isonB]) {	   //to save a little work, only lookup this
								//son if I don't already know him
								EmSon[isonB] = (Element*) El_Table->lookup(
										EmFather->son[isonB]);
								assert(EmSon[isonB]);
							}
							...
							EmSon[isonA]->neigh_proc[ineighm4] =
									EmSon[isonB]->neigh_proc[ineighm4] = neigh_proc;

							EmSon[isonA]->neigh_proc[ineighp4] =
									EmSon[isonB]->neigh_proc[ineighp4] = -2;

							NdTemp = (Node*) NodeTable->lookup(
									EmSon[isonA]->node_key[ineigh]);
							assert(NdTemp);
							NdTemp->info = CORNER;
							...
							NdTemp = (Node*) NodeTable->lookup(
									EmSon[isonB]->node_key[(ineigh + 1) % 4]);
							assert(NdTemp);
							NdTemp->info = CORNER;
	...
	for (ifather = RefinedList->get_inewstart();
		...
		for (isonA = 0; isonA < 4; isonA++) {
			...
			NdTemp = (Node*) NodeTable->lookup(
					EmSon[isonA]->node_key[(isonA + 1) % 4 + 4]);
			assert(NdTemp);
			NdTemp->info = SIDE;
		}
		...
		for (iside = 0; iside < 4; iside++) {
			...
			if (EmFather->neigh_proc[ineigh] == -1) {
				...
				EmNeighOld[1] = (Element*) El_Table->lookup(
						EmFather->neighbor[ineighp4]);
				assert(EmNeighOld[1]);
				...
				if (!(ineighme < 8)) {
					printf(
							...
							EmFather->key[0], EmFather->key[1], EmNeighOld[0]->key[0],
							EmNeighOld[0]->key[1], ineigh, isonA, isonB);
					printf("aborting!!");
				...
				case 0:
					...
					if (EmNeighOld[0]->adapted == OLDFATHER) {
						...
						EmNeighNew[0] = (Element*) El_Table->lookup(
								EmNeighOld[0]->son[(ineighme + 1) % 4]);
						assert(EmNeighNew[0]);

						EmNeighNew[1] = (Element*) El_Table->lookup(
								EmNeighOld[0]->son[ineighme]);
						assert(EmNeighNew[1]);
				...
				case 1:
					...
					if ((EmNeighOld[0]->adapted == OLDFATHER)
						...
						if (EmNeighOld[1]->adapted == OLDFATHER) {
							EmNeighNew[2] = (Element*) El_Table->lookup(
									EmNeighOld[1]->son[(ineighme + 1) % 4]);
							assert(EmNeighNew[2]);

							EmNeighNew[3] = (Element*) El_Table->lookup(
									EmNeighOld[1]->son[ineighme]);
							assert(EmNeighNew[3]);
						} else
				...
				case 1:
					...
					EmNeighNew[0]->neigh_proc[ineighme] =
							EmNeighNew[0]->neigh_proc[ineighmep4] = EmFather->myprocess;
				...
				case 2:
					...
					EmNeighNew[0]->neigh_proc[ineighme] =
							EmNeighNew[1]->neigh_proc[ineighme] = EmFather->myprocess;
					...
					EmNeighNew[0]->neigh_proc[ineighmep4] =
							EmNeighNew[1]->neigh_proc[ineighmep4] =
									EmNeighNew[2]->neigh_proc[ineighmep4] =
											EmNeighNew[3]->neigh_proc[ineighmep4] = -2;
					...
					if (ifrecvd) {
						...
						for (irecvd = 0; irecvd < num_recv[iproc]; irecvd++) {
							...
							EmTemp = (Element*) El_Table->lookup(
									&(recv[iproc][(4 * irecvd + 0) * KEYLENGTH]));
							/*
							...
							if (!EmTemp) {
								printf(
										"refine_neigh_update(): myid=%d receiving from iproc=%d, %dth element being received is {%10u,%10u} but this off processor neighbor is missing\n",
										myid, iproc, irecvd,
										recv[iproc][(4 * irecvd + 0) * KEYLENGTH + 0],
										recv[iproc][(4 * irecvd + 0) * KEYLENGTH + 1]);
								ElemBackgroundCheck(El_Table, NodeTable,
										&(recv[iproc][(4 * irecvd + 0) * KEYLENGTH]), stdout);
								assert(EmTemp);
							}
							...
							EmNeigh = (Element*) El_Table->lookup(
									&(recv[iproc][(4 * irecvd + 1) * KEYLENGTH]));
							if (EmNeigh) {
								if (!compare_key(&(recv[iproc][(4 * irecvd + 1) * KEYLENGTH]),
										EmNeigh->pass_key())) {
									printf(
											"refine_neigh_update(): myid=%d receiving from iproc=%d, %dth element being received is {%10u,%10u}\n That element is supposed to have an onprocessor element {%10u,%10u} as a neighbor but that \"on processor element\" thinks it key is {%10u,%10u}.\n",
											myid, iproc, irecvd,
											recv[iproc][(4 * irecvd + 0) * KEYLENGTH + 0],
											recv[iproc][(4 * irecvd + 0) * KEYLENGTH + 1],
									...
									ElemBackgroundCheck(El_Table, NodeTable,
											&(recv[iproc][(4 * irecvd + 0) * KEYLENGTH]), stdout);
									ElemBackgroundCheck(El_Table, NodeTable,
											&(recv[iproc][(4 * irecvd + 1) * KEYLENGTH]), stdout);
									ElemBackgroundCheck(El_Table, NodeTable, EmNeigh->pass_key(),
											stdout);
									assert(EmNeigh);
								}
							...
							if (EmTemp->adapted == OLDFATHER) {
								...
								for (ikey = 0; ikey < KEYLENGTH; ikey++) {
									EmSon[isonA]->neighbor[ineigh][ikey] =
											EmSon[isonA]->neighbor[ineighp4][ikey] = recv[iproc][(4
													* irecvd + 2) * KEYLENGTH + ikey];

									//bob
									EmSon[isonB]->neighbor[ineigh][ikey] =
											EmSon[isonB]->neighbor[ineighp4][ikey] = recv[iproc][(4
													* irecvd + 3) * KEYLENGTH + ikey];
								}
								...
								EmSon[isonA]->neigh_proc[ineigh] =
										EmSon[isonB]->neigh_proc[ineigh] = iproc;
								...
								if (EmTemp->neigh_gen[ineigh] == EmTemp->generation) {
									...
									if (EmTemp->neigh_gen[(ineigh + 3) % 4]
											== EmTemp->generation) {
										inode = ineigh;
										NdTemp = (Node*) NodeTable->lookup(EmTemp->node_key[inode]);
										assert(NdTemp);
									...
									if (EmTemp->neigh_gen[(ineigh + 1) % 4]
											== EmTemp->generation) {
										inode = (ineigh + 1) % 4;
										NdTemp = (Node*) NodeTable->lookup(EmTemp->node_key[inode]);
										assert(NdTemp);
								...
								for (ikey = 0; ikey < KEYLENGTH; ikey++) {
									EmTemp->neighbor[ineigh][ikey] = recv[iproc][(4 * irecvd + 2)
											* KEYLENGTH + ikey];

									EmTemp->neighbor[ineighp4][ikey] =
											recv[iproc][(4 * irecvd + 3) * KEYLENGTH + ikey];
								}
								EmTemp->neigh_gen[ineighp4] = EmTemp->neigh_gen[ineigh] =
								...
								if (EmTemp->generation > EmTemp->neigh_gen[ineigh]) {
									NdTemp = (Node*) NodeTable->lookup(
											EmTemp->node_key[EmTemp->get_which_son()]);
									assert(NdTemp);
									NdTemp->info = CORNER;
								}
...
extern void update_neighbor_interprocessor(HashTable* HT_Elem_Ptr,
...
void update_neighbor_info(HashTable* HT_Elem_Ptr, ElemPtrList* RefinedList,
	...
	for (int i = 0; i < RefinedList->get_num_elem(); i++) {
		...
		assert(
				(EmTemp->get_adapted_flag()==OLDFATHER)|| (EmTemp->get_adapted_flag()==TOBEDELETED));
		...
		if (EmTemp->get_adapted_flag() == OLDFATHER) {
			...
			for (int j = 0; j < 8; j++) {
				if ((*(orig_neigh_proc + j) >= 0) && (*(orig_neigh_proc + j) == myid)) //neighbor is at the same proc.
						{
					Neighbor = (Element*) HT_Elem_Ptr->lookup(
							orig_neighbors + j * KEYLENGTH);
					assert(Neighbor);
					...
					if (!NeighRefined) {
						...
						Neighbor->change_neighbor(&NewNeighbor[0][0], which_side, myid,
								reg);
					}
					...
					else //-- neighbor is refined
					{
						if (NeighGeneration == (MyGeneration + 1)) //--case 4
							...
							NdTemp = (Node*) (HT_Node_Ptr->lookup(
									Neighbor->getNode() + (which_side + 4) * KEYLENGTH));
							NdTemp->putinfo(S_C_CON);
							//NdTemp->putorder
							...
							for (k = 0; k < KEYLENGTH; k++)
								NewNeighbor[0][k] = NewNeighbor[1][k] = *(Sons + a * KEYLENGTH
										+ k);
							...
							for (k = 0; k < KEYLENGTH; k++)
								NewNeighbor[0][k] = NewNeighbor[1][k] = *(NeighSons
										+ which_side * KEYLENGTH + k);
							...
							if (b == 4)
								b = 0;
							Neighbor = (Element*) (HT_Elem_Ptr->lookup(
									NeighSons + a * KEYLENGTH));

							int edge_of_son;
							if (a == which_side)
								edge_of_son = which_side;
							else
							...
							Neighbor->change_neighbor(&NewNeighbor[0][0], edge_of_son, myid,
									3);
				...
				else if (*(orig_neigh_proc + j) != myid
						&& *(orig_neigh_proc + j) >= 0) {
					...
					if (NeighGeneration == MyGeneration) {
						assert(j < 4);
						refined_new->set_parameters(*(orig_neigh_proc + j),
								(orig_neighbors + j * KEYLENGTH), Mykey, Sons, SIDE_SONS[j],
								MyGeneration, 1);
					}

					else
						refined_new->set_parameters(*(orig_neigh_proc + j),
								(orig_neighbors + j * KEYLENGTH), Mykey, Sons,
								&SIDE_SONS[j % 4][j / 4], MyGeneration, 2); //the neighbor is younger...
	...
	MPI_Barrier(MPI_COMM_WORLD);
	update_neighbor_interprocessor(HT_Elem_Ptr, HT_Node_Ptr, refined_start, myid,
			nump);
