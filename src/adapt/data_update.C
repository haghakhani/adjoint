...
void write_node_info_ext(HashTable* HT_Node_Ptr, Element* Em, int start,
		int mid);

void data_update(HashTable* HT_Elem_Ptr, HashTable* HT_Node_Ptr, Recv* RecvHead,
		...
		if (recv->sender_gen == EmTemp->get_gen()) {
			...
			if (EmTemp->get_refined_flag()) //-- if the receptor was refined
			{
				...
				else /*--both sender and receptor are not refined and they are on the same generation--*/
				{
					...
					p = HT_Node_Ptr->lookup(
							EmTemp->getNode() + (recv->side + 4) * KEYLENGTH); //-- cc
					assert(p);
					Node* NdTemp = (Node*) p;
		...
		else //-- sender's generation is diffrent from receptor's
		{
			...
			if (recv->sender_gen > EmTemp->get_gen()) //--sender is smaller than receptor
				...
				} else //-- p enrichment
				{
					...
					Node* NdTemp = (Node*) (HT_Node_Ptr->lookup(
							ndkey + (start + 4) * KEYLENGTH));
			...
			if (recv->sender_gen < EmTemp->get_gen()) //--sender is bigger than receptor
				...
				if (recv->sender_refined) {
					...
					Node* NdTemp =
							(Node*) (HT_Node_Ptr->lookup(ndkey + start * KEYLENGTH));
...
void write_node_info_ext(HashTable* HT_Node_Ptr, Element* Em, int start,
	...
	NdTemp = (Node*) (HT_Node_Ptr->lookup(keyP + start * KEYLENGTH));
	assert(NdTemp);
	NdTemp->putinfo(CORNER);
	NdTemp->put_order(1);

	NdTemp = (Node*) (HT_Node_Ptr->lookup(keyP + mid * KEYLENGTH));
	assert(NdTemp);
	NdTemp->putinfo(SIDE);
